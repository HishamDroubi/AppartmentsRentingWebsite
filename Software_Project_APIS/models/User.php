...
class User
{
    ...
    public function signUpAdmin()
    {
        try {
            ...
            //
            $sql = 'INSERT INTO `admin`(`id`, `password`,`status`,`token`) VALUES (?,?,?,?)';
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(1, $this->admin_id);
    ...
    function getTokens()
    {
        $sql = 'SELECT  `token` FROM `managers` ';
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        $tokens = array();
        if ($stmt->rowCount() > 0) {
    ...
    function sendGCM($type, $token)
    {
        $API_KEY = "AAAAjbQNtt0:APA91bFZGaz8edRh3gDcTdwzwwG_ywZGDtFOZRqDgabR0CFcB1yH-RPaYHXTZWrmofsricVXbEN1FTqaZVsdxOeO8oaXzeDt_MVxklFl97G5NHkt9ssIaxLcXagdK6eP5Rnv75f8Hmen";
